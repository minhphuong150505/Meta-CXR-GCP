...
class CrossModalAttentionLayerWithPartialTraining(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.1, text_dropout_rate=0.4):
        super(CrossModalAttentionLayerWithPartialTraining, self).__init__()
        
        # Cross-attention layers for image and query interactions
        self.image_to_query_attention = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, image_patches, text_embeddings=None, expert_tokens=None, query_embeddings = None):
        ...
        # Cross-attend image patches with query embeddings
        attended_image, _ = self.image_to_query_attention(
            query=image_patches, key=query_embeddings, value=query_embeddings
        )
        ...
        expert_to_fused, expert_attention = self.expert_to_fused_attention(
            query=attended_experts, key=fused_representation, value=fused_representation
        )
        ...
        return expert_features, expert_attention
...
class AbnormalityClassifierWithPartialTraining(nn.Module):
    ...
    def forward(self, image_patches, text_embeddings = None, query_embeddings = None):
        ...
        for layer in self.cross_modal_attention_layers:
            expert_tokens, expert_attention = layer(img_proj, txt_proj, expert_tokens, query_proj)
        ...
        return logits, expert_attention
