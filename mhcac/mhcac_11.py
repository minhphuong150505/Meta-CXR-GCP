...
class ExpertTokenCrossAttention(nn.Module):
    ...
    def forward(self, expert_tokens, image_patches, text_embeddings = None):
        if text_embeddings is not None:
            ...
            expert_text = self.norm_expert_text(expert_tokens)
        
        # Expert-to-ViT cross-attention
        expert_image, attention_weights = self.expert_to_image_attention(query = expert_text, key = image_patches, value = image_patches)
        expert_image = self.norm_expert_image(expert_image + expert_text)  # Residual connection
        ...
        expert_refined, _ = self.self_attention(
            query=expert_image, key=expert_image, value=expert_image
        )
        expert_refined = self.norm_self_attention(expert_image + expert_refined)  # Residual connection and normalization
